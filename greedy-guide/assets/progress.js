...
(function () {
  ...
  function setStatus(id, status) {
    ...
    if (currentUser && db) {
      ...
      db.collection("users").doc(currentUser.uid).set(update, { merge: true })
        .catch(function (e) { console.error("[GreedyProgress] 寫入雲端失敗：", e); });
    }
  }
  ...
  function renderSummary() {
    ...
    if (!el) return;
    var s = summary();
    var total = (window.GREEDY_PROBLEMS && window.GREEDY_PROBLEMS.length) || (s.none + s.review + s.done);
      ...
      .then(function () {
        ...
        auth.onAuthStateChanged(function (user) {
          ...
          unsubscribeDoc = ref.onSnapshot(function (snap) {
            ...
            paint();
          }, function (err) { console.error("[GreedyProgress] 同步監聽失敗：", err); });
        });
      })
      .catch(function (e) {
        console.error("[GreedyProgress] Firebase 載入失敗：", e);
        renderAuthUI();
      });
  }
  ...
  window.GreedyProgress = { getStatus: getStatus, setStatus: setStatus, paint: paint, summary: summary };
})();
